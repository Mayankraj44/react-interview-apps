...
const createGrid=(size,disabledList)=>{
    return Array(size).fill().map((_,rowIndex)=>Array(size).fill()?.map((_,colIndex)=>{
        console.log(disabledList)
        
        if(disabledList.find(val=> val[0]===rowIndex && val[1]===colIndex)){
            return null
        }
        return 0
    }))
        ...
        {tiles?.map((row,rowIndex)=>(
            <div className='row'>
                {row?.map((val,colIndex)=>(
                    <div className={`tile ${val ? 'active' :'inactive'} ${val === null && 'disabled'}` } onClick={()=>!val && !reversingFlag && handleTileClick(rowIndex,colIndex) } />
                ))}
            </div>
        ))}
    </div>
